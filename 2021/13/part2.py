...
def read_from_file(fp:str) -> (list, list):
    with open(fp, 'r') as f:
        ...
        height = max_y + 3 if max_y % 2 != 0 else max_y
...
def apply_folds(folds: list, manual: list) -> list:
...
def print_manual(folded_manual: list) -> None:
    dot_count = 0
    for line in folded_manual:
        print(''.join(['.' if num == 0 else '#' for num in line]))

if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='Advent of code 2021 - day 13, part 2')
    parser.add_argument('input', help='path to input file', default='input.txt', nargs='?')
    args = parser.parse_args()
    manual, folds = read_from_file(args.input)
    folded_manual = apply_folds(folds, manual)
    print_manual(folded_manual)