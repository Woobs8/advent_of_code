...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='Advent of code 2021 - day 4, part 1')
    parser.add_argument('input', help='path to input file', default='input.txt', nargs='?')
    args = parser.parse_args()
    boards, draws = read_from_file(args.input)
    board_index = index_boards(boards)
    winning_board, draw = play(boards, board_index, draws)
    score = sum([sum(line) for line in winning_board]) * draw
    print(score)