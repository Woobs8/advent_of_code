...
def deduplicate(velocities: list) -> set:
    unique_velocities = set()
    for velocity in velocities:
        vx, vy, __ = velocity
        unique_velocities.add((vx,vy))
    return unique_velocities

if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='Advent of code 2021 - day 17, part 2')
    parser.add_argument('input', help='path to input file', default='input.txt', nargs='?')
    args = parser.parse_args()
    target_area = read_from_file(args.input)
    velocities = find_valid_velocities(target_area)
    unique_velocities = deduplicate(velocities)
    print(len(unique_velocities))
