...
def calc_fuel_cost(positions: list, target_position: int) -> int:
    ...
    for position in positions:
        fuel_cost += calc_sum_of_numbers_in_interval(1, abs(target_position - position))
    return fuel_cost

def calc_sum_of_numbers_in_interval(start: int, end: int) -> int:
    return ((start + end) * end) // 2

if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='Advent of code 2021 - day 7, part 2')
