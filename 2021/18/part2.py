...
def find_highest_magnitude_sum(numbers: list) -> int:
    magnitude = 0
    for a, b in permutations(numbers,2):
        reduced_sum = reduce_number(add(a.copy(), b.copy()))
        permutation_magnitude = reduced_sum.get_magnitude()
        if permutation_magnitude > magnitude:
            magnitude = permutation_magnitude
    return magnitude
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='Advent of code 2021 - day 18, part 2')
    parser.add_argument('input', help='path to input file', default='input.txt', nargs='?')
    args = parser.parse_args()
    numbers = read_from_file(args.input)
    highest_mangitude_sum = find_highest_magnitude_sum(numbers)
    print(highest_mangitude_sum)